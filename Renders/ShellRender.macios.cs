    ...
    protected override IShellPageRendererTracker CreatePageRendererTracker()
    {
        if (Tracker is not null)
            throw new InvalidOperationException(
                "This should have been cleared out by CustomShellSectionRenderer"
            );
...
public class CustomShellSectionRootHeader : ShellSectionRootHeader
{
    ...
    public CustomShellSectionRootHeader(IShellContext shellContext)
        : base(shellContext) { }
...
public class CustomShellSectionRootRenderer : ShellSectionRootRenderer
{
    ...
    public CustomShellSectionRootRenderer(
        ShellSection shellSection,
        IShellContext shellContext,
        CustomShellSectionRenderer customShellSectionRenderer
    )
        : base(shellSection, shellContext)
    {
        ShellSection = shellSection;
    ...
    protected override IShellSectionRootHeader CreateShellSectionRootHeader(
        IShellContext shellContext
    )
    {
...
public class CustomShellSectionRenderer : ShellSectionRenderer
{
    ...
    public CustomShellSectionRenderer(IShellContext context)
        : base(context)
    {
    ...
    protected override IShellSectionRootRenderer CreateShellSectionRootRenderer(
        ShellSection shellSection,
        IShellContext shellContext
    )
    {
        return _customShellSectionRootRenderer = new CustomShellSectionRootRenderer(
            shellSection,
            shellContext,
            this
        );
    ...
    void UpdateLargeTitle()
    {
        ...
        Invoke(
            ...
            {
                if (
                    _customShellSectionRootRenderer?.CustomShellSectionRootHeader
                    is UICollectionViewController uvcvc
                )
                {
...
public class CustomShellPageRendererTracker : ShellPageRendererTracker
{
    ...
    Page? ToolbarCurrentPage
    {
        get
        {
            ...
            var currentPageProperty = toolbarType?.GetField(
                "_currentPage",
                BindingFlags.Public
    ...
    void UpdateBackButtonTitle()
    {
        ...
        var text = behavior.GetPropertyIfSet<string?>(
            BackButtonBehavior.TextOverrideProperty,
            null
        );
        ...
        if (navController is not null)
        {
            var viewControllers =
                ...
                ?? throw new InvalidOperationException(
                    $"{nameof(ViewController.NavigationController.ViewControllers)} cannot be null"
                );
            ...
            if (count > 1 && viewControllers[count - 1] == ViewController)
            {
                ...
                if (previousNavItem is not null)
                {
                    ...
                    {
                        var barButtonItem = previousNavItem.BackBarButtonItem ??=
                            new UIBarButtonItem();
...
{
    public CustomTitleViewContainer(View view)
        : base(view)
    {
        MatchHeight = true;
    ...
    void UpdateFrame(UIView newSuperView)
    {
        if (newSuperView.Bounds != CGRect.Empty)
        {
            if (
                ...
                Frame = new CGRect(
                    Frame.X,
                    newSuperView.Bounds.Y,
                    Frame.Width,
                    newSuperView.Bounds.Height
                );
...
{
    public static readonly BindableProperty PrefersLargeTitlesProperty = BindableProperty.Create(
        "PrefersLargeTitles",
        typeof(bool),
        typeof(ShellAttachedProperties),
        false
    );

    public static bool GetPrefersLargeTitles(BindableObject element) =>
        (bool)element.GetValue(PrefersLargeTitlesProperty);
    ...
    public static IPlatformElementConfiguration<iOS, Shell> SetPrefersLargeTitles(
        this IPlatformElementConfiguration<iOS, Shell> config,
        bool value
    )
    {
